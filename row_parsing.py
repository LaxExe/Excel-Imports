...
def gather_row_data(excel_file, json_structure):
  ...
  if json_structure["address_1_column_format"] != None:
  ...
  if is_1_column_tag("info.json"):
        ...
        for i in range(2, last_row + 1):
          ...
          # Get Additional Fields using the column_name from JSON
          items = []
          for key, field_data in additional_fields.items():
              if field_data is not None:
                  col_index = col_indexes.get(key)
                  if col_index:
                      cell_value = ws.cell(row=i, column=col_index).value
                      column_name = field_data.get("column_name", key)
                      items.append(f"{column_name}: {cell_value}")
    ...
    for i in range(2, last_row + 1):
      ...
      formatted_address = address_result["address"]
      needs_ai = address_result["needs_ai"]
      ...
      # Process additional fields using the column name
      items = []
      additional_fields = json_structure.get("additional_fields", {})
      for key, field_data in additional_fields.items():
          if field_data is not None:
