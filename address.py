        ...
        if location:
            ...
            for part in geocoded_parts:
                ...
                # Detect provinces/states (usually 2-3 letter codes or full names)
                    ...
                    detected_fields['province_or_statename'] = part
...
def column_1_address_skip(address, format_str, separator):
    """
    This function handles single column address processing with geopy

    Args:
        address: single column addresss string
        format_str: expect format - "street, city, province, postol code"
        seperator: seperator characters such as ","
    ...
    # Map format parts to standard field names. 
    field_mapping = {
    ...
    if extra_count >= 0:
        for i, key in enumerate(format_parts):
            ...
            if len(value) < 2 or len(value) > 40:
                # Mark as missing information for geopy processing
                needs_ai[field_name] = True
                result[field_name] = " "

            else:
                result[field_name] = value

         # Check for missing required fields that weren't in the format
        all_fields = ["street_address", "city", "province", "postal_code", "country"]
    ...
    # this handles case where address has fewer parts than format
    else:
        # Handle case where we have fewer address parts than format parts 
        for i, address_part in enumerate (address_part):
            if i < len(format_parts):
                key = format_parts[i]
                field_name = field_mapping.get (key, key.lower())

                # Validate the address 
                if len (address_part) < 3 or len (address_part) > 40:
                    # Need geopy here 
                    needs_ai [field_name] = True
                    result [field_name] = " "

                else:
                    result [field_name] = address_part   

        # We mark remaining format parts as missing - so if format expects 5 and we have 2. We have 3 missing.
        for i in range (len(address_part), len (format_parts)):
            key = format_parts[i]
            field_name = field_mapping.get (key, key.lower())
            needs_ai[field_name] = True
            result[field_name] = " "   
    ...
    # Try to fill missing field using geopy
    if needs_ai:

        # Create Address dict for geopy - dummy columns 
        address_fields = {
        "street_address": "A",      
        "city": "B",                         
        "province_or_statename": "C",               
        "postal_code": "D",                
        "country": "E"              
    }

    # Calling the geopy function 
    geopy_results = fill_missing_address_with_geopy(result, needs_ai, address_fields)
    
    if isinstance (geopy_results, tuple) and len (geopy_results) == 2:
        filled_parts, remaining_missing = geopy_results
        # Update the results with filled fields. 
...
def column_multi_address(row, row_number, json_path="info.json"):
    ...
    address_fields = {
        ...
        "province_or_statename": additional_cols.get("province_or_statename"),               
    ...
    for field, col_letter in address_fields.items():
        if col_letter:  # Field exists in JSON configuration
            ...
            # Column doesn't exist in JSON configuration at all – skip it
    ...
    ordered_fields = ["street_address", "city", "province_or_statename", "postal_code", "country"]
